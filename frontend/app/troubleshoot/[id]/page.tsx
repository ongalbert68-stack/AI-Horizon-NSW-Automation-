...
import { createCheckResult, getCase, updateCase } from "@/lib/api/cases";
...
const STAGE_HINTS: Record<StepId, Phase> = {
  ...
  loop_check_cause: { n: 4, label: "Troubleshooting", hint: "Pick which candidate cause you want to test." },
...
export default function CaseWizardPage() {
  ...
  async function persistAxes(caseId: number) {
    try {
      ...
      // best-effort — a reload retries via the axes already on the case
    }
  }
  ...
  async function runStep(step: StepId, caseData: Case, arg?: unknown) {
    ...
    switch (step) {
      ...
      case "ask_axis_other": {
        ...
        setPending({ kind: "text", placeholder: "e.g. it leaves a hair between the pads", confirmLabel: "Next" });
        return;
      }
      ...
      case "loop_check_cause": {
        ...
        const options: ChipOption[] = (ranking?.pass1 ?? []).slice(0, 4).map((c) => ({ value: c.cause_id, label: c.label }));
        say("Which candidate cause do you want to test?");
      ...
      case "loop_check_detail": {
        ...
        setPending({ kind: "text", placeholder: "e.g. air slug expelled", confirmLabel: "Log it", allowEmpty: true });
        return;
      }
      case "loop_submit": {
        setBusy(true);
        try {
          ...
          // NSW-Automation's POST /cases/{id}/check-results returns the
          // created CheckResult, not a re-ranking (pipeline's endpoint
          // re-ranks server-side and returns Ranking) — so this re-ranks
          // with a separate call instead of trusting the create response.
          await createCheckResult(caseData.case_id, {
            check_name: draftCheckRef.current.check_name!,
            cause_id: draftCheckRef.current.cause_id!,
          ...
          const updated = await rankCase(caseData.case_id);
          ...
          say(
            `Logged — re-ranked. Top candidate now: ${updated.pass1[0].label} at ` +
              `${Math.round(updated.pass1[0].likelihood * 100)}%. ` +
      ...
      case "close": {
        ...
        try {
          ...
          say(
            `Closed as ${result.tier}.` +
              (result.gaps.length > 0 ? ` Gaps: ${result.gaps.join("; ")}.` : " Every condition was met."),
          );
        } catch (err) {
  ...
  React.useEffect(() => {
    ...
    Promise.all([getCase(caseId), getIntakeSpec()])
      .then(([caseData, questions]) => {
        ...
        // Gated on `!caseData.resolved`, matching the case detail page's own
        // status badge (see [id]/page.tsx's "Continue troubleshooting" link)
        // rather than a separate `closed_at` check that can drift from it —
        // pipeline's equivalent gate uses `closed_at` while its badge reads
        // `resolved`, which is the bug this port deliberately avoids.
        if (caseData.closed_at) {
          setStage("done");
          say(caseData.tier ? `This case is already closed as ${caseData.tier}.` : "This case is already closed.");
          setPending({ kind: "none" });
          return;
        }
